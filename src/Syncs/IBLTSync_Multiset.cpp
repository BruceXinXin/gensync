...
bool IBLTSync_Multiset::SyncClient(const shared_ptr<Communicant>& commSync, list<shared_ptr<DataObject>> &selfMinusOther, list<shared_ptr<DataObject>> &otherMinusSelf){
    try {
        ...
        if(!oneWay) {
            ...
            stringstream msg;
            msg << "[client]IBLTSync_Multiset succeeded." << endl;
...
bool IBLTSync_Multiset::SyncServer(const shared_ptr<Communicant>& commSync, list<shared_ptr<DataObject>> &selfMinusOther, list<shared_ptr<DataObject>> &otherMinusSelf){
    try {
        ...
        for(const auto& pair : positive) {
            otherMinusSelf.push_back(make_shared<DataObject>(pair.first));
        }
...
bool IBLTSync_Multiset::addElem(shared_ptr<DataObject> datum){
    // call parent add
    SyncMethod::addElem(datum);
    myIBLT.insert(datum->to_ZZ(), datum->to_ZZ());
    return true;
}
bool IBLTSync_Multiset::delElem(shared_ptr<DataObject> datum){
    // call parent delete
