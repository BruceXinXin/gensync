...
InterCPISync::InterCPISync(long m_bar, long bits, int epsilon, int partition,
    ...
    : maxDiff(m_bar), bitNum(bits), pFactor(partition), hashes(Hashes),
      probEps(conv<int>(ceil(
          -log10(
              (RR_ONE -
               pow(RR_ONE - pow(RR_TWO, (RR)-epsilon),
                   RR_ONE / (RR_ONE +
                             pow(RR_TWO, (RR)bits) * (RR)partition / (RR)m_bar *
                                 (RR)ceil(bits * log(2) / log(partition)))))) /
          log10(RR_TWO)))) {

    /**
     * ProbEps: Derivation
     * Probability of failure for each CPISync node = 2^-Epsilon
     * 1-(1-P[error per node])^maxNodes <= 2^-Epsilon //Probability of 1 or more
     * errors occuring: max nodes for InterCPISync = 1 +
     * (symDifs*partition/m_bar)*ceil(bits* log_p(2)) Then solve for P[error per
     * node] to find the epsilon_0(Epsilon for each CPISync node) needed for
     * each CPISync in order to bound the total error by the given epsilon
     */
...
bool InterCPISync::delElem(shared_ptr<DataObject> datum) {
    ...
    if (!SyncMethod::delElem(datum))
        return false; // run the parent's version first

    Logger::gLog(Logger::METHOD_DETAILS,
                 ". (InterCPISync) removing item " + datum->print());
...
bool InterCPISync::addElem(shared_ptr<DataObject> newDatum) {
    ...
    if (!SyncMethod::addElem(newDatum))
        return false; // Run parents version

    addElemHashID = rep(_hash(newDatum)); // compute the hash of the item for
                                          // use in the recursive addElem

    Logger::gLog(
        Logger::METHOD_DETAILS,
        ". (InterCPISync) adding item " + newDatum->print() +
            " with representation = " + toStr(addElemHashID)); // log the action

    if (treeNode == nullptr)
        treeNode = new pTree(new CPISync_ExistingConnection(
                                 maxDiff, bitNum, probEps, redundant_k, hashes),
                             pFactor);

    CPISync *curr = treeNode->getDatum();
    return curr->addElem(newDatum);
    //  return addElem(newDatum, treeNode, NULL, ZZ_ZERO, DATA_MAX); // use the
    //  recursive helper method
}

bool InterCPISync::SyncClient(const shared_ptr<Communicant> &commSync,
    ...
    // 1. Do the sync
    pTree *parentNode = treeNode; // Create a copy of the root node - Just to
                                  // make sure that it is not deleted
    commSync->hardResetCommCounters(); // Because each CPISync will reset the
                                       // communicant stats need to reset and
                                       // use the "total" fields
    bool result =
        SyncMethod::SyncClient(
            commSync, selfMinusOther,
            otherMinusSelf) // also call the parent to establish bookkeeping
                            // variables
        && _SyncClient(commSync, selfMinusOther, otherMinusSelf, parentNode,
                       ZZ_ZERO,
                       DATA_MAX); // Call the modified Sync with data Ranges

    if (result) { // Sync succeeded
        Logger::gLog(
            Logger::METHOD,
            string("Interactive sync succeeded.\n") + "   self - other =  " +
                printListOfSharedPtrs(selfMinusOther) + "\n" +
...
void InterCPISync::SendSyncParam(const shared_ptr<Communicant> &commSync,
                                 bool oneWay /* = false */) {
...
void InterCPISync::RecvSyncParam(const shared_ptr<Communicant> &commSync,
                                 bool oneWay /* = false */) {
    ...
    if (theSyncID != enumToByte(SyncID) || mbarClient != maxDiff ||
        bitsClient != bitNum || epsilonClient != probEps ||
        pFactor != pFactorClient) {
        // report a failure to establish sync parameters
        commSync->commSend(SYNC_FAIL_FLAG);
        Logger::gLog(
            Logger::COMM,
            "Sync parameters differ from client to server: Client has (" +
                toStr(mbarClient) + "," + toStr(bitsClient) + "," +
                toStr(epsilonClient) + "," + toStr(pFactorClient) +
                ").  Server has (" + toStr(maxDiff) + "," + toStr(bitNum) +
                "," + toStr(probEps) + "," + toStr(pFactor) + ").");
        throw SyncFailureException("Sync parameters do not match.");
    }
...
bool InterCPISync::SyncServer(const shared_ptr<Communicant> &commSync,
                              list<shared_ptr<DataObject>> &selfMinusOther,
                              list<shared_ptr<DataObject>> &otherMinusSelf) {
    ...
    commSync->hardResetCommCounters(); // Because each CPISync will reset the
                                       // communicant stats need to reset and
                                       // use the "total" fields
    result &= _SyncServer(commSync, selfMinusOther, otherMinusSelf, parentNode,
                          ZZ_ZERO, DATA_MAX);
    if (result) { // Sync succeeded
        Logger::gLog(
            Logger::METHOD,
...
ZZ_p InterCPISync::_hash(shared_ptr<DataObject> datum) const {
    ...
    return to_ZZ_p(num %
                   DATA_MAX); // reduce to bit_num bits and make into a ZZ_p
}

bool InterCPISync::_createTreeNode(pTree *&treeNode, pTree *parent,
                                   const ZZ &begRange, const ZZ &endRange) {
    ...
    if (parent != nullptr) {
        ...
        for (auto elem = par->beginElements(); elem != par->endElements();
             elem++) {
            const ZZ elemZZ = rep(_hash(*elem));
            if ((elemZZ >= begRange) && (elemZZ < endRange) &&
                (!curr->addElem(*elem))) // if element is in range and add fails
                return false;
        }
    }
    return true;
...
bool InterCPISync::_addElem(shared_ptr<DataObject> newDatum, pTree *&treeNode,
                            pTree *parent, const ZZ &begRange,
                            const ZZ &endRange) {
    Logger::gLog(Logger::METHOD, "Entering InterCPISync::addElem");
    Logger::gLog(Logger::METHOD_DETAILS, ".  (InterCPISync) adding in range " +
                                             toStr(begRange) + " - " +
                                             toStr(endRange));
    CPISync *curr;
    ...
    if (treeNode == nullptr) {
        _createTreeNode(treeNode, parent, begRange,
                        endRange); // create a new tree node here
        curr = treeNode->getDatum();

        if (parent == nullptr) // I'm at the root of the tree ... nothing was
                               // propagated, so add the new datum
            if (!curr->addElem(newDatum))
                return false;
    } else { // treeNode was not null ... just add the newDatum
        curr = treeNode->getDatum();
        if (!curr->addElem(newDatum))
            return false; // Add to current node
    }

    // do we need to add to its children as well?
    if (curr->getNumElem() > 0 &&
        endRange - begRange >
            1) { // keep dividing until we have no elements or we reach a cell
                 // with only one value in its range
        // find the correct child
        ZZ step = (endRange - begRange) / pFactor;

        if (step == 0) { // minimum step size is 1 - this will leave some
                         // partitions unused
            step = 1;
        }
        ...
        if (childPos >= pFactor)
            childPos =
                pFactor - 1; // lump all final elements into the last child

        ZZ newBegin =
            begRange +
            childPos *
                step; // beginning point of the range of the appropriate child
        ZZ newEnd =
            (childPos == pFactor - 1)
                ? endRange
                : // last elements lumped into the last child
                min(newBegin + step,
                    endRange); // don't overrun the end range of the parent node

        // recursively add newDatum to the appropriate child
        if (!_addElem(newDatum, treeNode->child[childPos], treeNode, newBegin,
                      newEnd))
            return false;
        ...
        for (long ii = 0; ii < pFactor - 1; ii++)
            if (treeNode->child[ii] == nullptr)
                _createTreeNode(treeNode->child[ii], treeNode,
                                begRange + ii * step,
                                begRange + (ii + 1) * step);

        if (treeNode->child[pFactor - 1] == nullptr) // last case is special
            _createTreeNode(treeNode->child[pFactor - 1], treeNode,
                            begRange + (pFactor - 1) * step, endRange);
    }
    return true;
}

bool InterCPISync::_delElem(shared_ptr<DataObject> datum, pTree *&node,
                            const ZZ &begRange, const ZZ &endRange) {
    // Compute the hash of the element to find out which children to search if
    // it is present in the parent
    addElemHashID = rep(_hash(datum));

    // If you are in the parent node and delete fails the element is not in your
    // tree
    if (node == treeNode) {
        // Only print logger details when you first enter (When node ==
        // parentNode)
        Logger::gLog(Logger::METHOD,
                     "Entering recursive InterCPISync::delElem");
        Logger::gLog(Logger::METHOD_DETAILS,
                     ". (InterCPISync) removing item recursively" +
                         datum->print());
        if (!node->getDatum()->delElem(datum))
            return false;
    }
    // If you've deleted the last element from the branch and you are not in the
    // parent node you have succeeded
    if (node->getDatum()->getNumElem() == 0) {
        return true;
    }

    ZZ step = (endRange - begRange) /
              pFactor; // Calculate the size of the bin of one node

    if (step ==
        0) { // minimum step size is 1 - this will leave some partitions unused
        step = 1;
    }
    ...
    ZZ newBegin =
        ...
        childPos *
            step; // beginning point of the range of the appropriate child
    ZZ newEnd =
        (childPos == pFactor - 1) ? endRange
                                  : // last elements lumped into the last child
            min(newBegin + step,
                endRange); // don't overrun the end range of the parent node

    // Recurse if the child pointer has elements in it
    if (node->child[childPos] != nullptr &&
        node->child[childPos]->getDatum()->getNumElem() != 0) {
...
bool InterCPISync::_SyncClient(const shared_ptr<Communicant> &commSync,
                               list<shared_ptr<DataObject>> &selfMinusOther,
                               list<shared_ptr<DataObject>> &otherMinusSelf,
                               pTree *&treeNode) {
    Logger::gLog(Logger::METHOD, "Entering InterCPISync::SyncClient");
    try {
        ...
        if (treeNode == nullptr) { // Case 0:  I've got nothing; the other must
                                   // have something
            Logger::gLog(Logger::METHOD_DETAILS, "My node is empty");
            commSync->commSend(SYNC_NO_INFO);
            response = commSync->commRecv_byte(); // get the other Communicant's
                                                  // initial declaration

            if (response !=
                SYNC_NO_INFO) // it is not the case that both nodes are empty
        ...
        response = commSync->commRecv_byte(); // get the other Communicants
                                              // initial declaration

        Logger::gLog(Logger::METHOD_DETAILS,
                     "My node has " + toStr(node->getNumElem()) + " elements.");
        Logger::gLog(Logger::COMM, " ... data is " + node->printElem());
        if (response ==
            SYNC_NO_INFO) { // Case 1:  I have something; the other has nothing
            node->sendAllElem(commSync, selfMinusOther); // send all I've got
            return true;
        } else { // Case 2: We both have something
            // synchronize the current node
            node->SyncClient(commSync, selfMinusOther,
                             otherMinusSelf); // attempt synchroniztion
            if (commSync->commRecv_byte() ==
                SYNC_FAIL_FLAG) { // i.e. the sync is reported by the Server to
                                  // have failed; recurse
                Logger::gLog(Logger::METHOD_DETAILS,
                             " > dividing into children");
                // synchronize the children, in order
                for (int ii = 0; ii < pFactor; ii++) {
                    Logger::gLog(Logger::METHOD_DETAILS,
                                 "  CHILD: " + toStr(ii));
                    _SyncClient(commSync, selfMinusOther, otherMinusSelf,
                                treeNode->child[ii]);
                }
                Logger::gLog(Logger::METHOD_DETAILS,
...
bool InterCPISync::_SyncServer(const shared_ptr<Communicant> &commSync,
                               list<shared_ptr<DataObject>> &selfMinusOther,
                               list<shared_ptr<DataObject>> &otherMinusSelf,
                               pTree *&treeNode) {
    ...
    if (treeNode == nullptr ||
        treeNode->getDatum()->getNumElem() ==
            0) { // Case 0:  I've got nothing; the other must have something
        Logger::gLog(Logger::METHOD_DETAILS, "My node is empty");
        commSync->commSend(SYNC_NO_INFO);
        response = commSync->commRecv_byte(); // get the other Communicants
                                              // initial declaration
    ...
    response =
        commSync
            ->commRecv_byte(); // get the other Communicants initial declaration
    ...
    if (response ==
        ...
        if (!node->SyncServer(
                commSync, selfMinusOther,
                otherMinusSelf)) { // sync failure - go on with children
            commSync->commSend(
                SYNC_FAIL_FLAG); // notify the client to try the children

            Logger::gLog(Logger::METHOD_DETAILS, " > dividing into children");
            // synchronize the children, in order
            for (int ii = 0; ii < pFactor; ii++) {
                Logger::gLog(Logger::METHOD_DETAILS, "  CHILD: " + toStr(ii));
                _SyncServer(commSync, selfMinusOther, otherMinusSelf,
                            treeNode->child[ii]);
            }
            Logger::gLog(Logger::METHOD_DETAILS, "< returning from division");

        } else
            commSync->commSend(SYNC_OK_FLAG); // notify the client that sync
                                              // succeeded ... do not recurse
...
bool InterCPISync::_SyncServer(const shared_ptr<Communicant> &commSync,
                               list<shared_ptr<DataObject>> &selfMinusOther,
                               list<shared_ptr<DataObject>> &otherMinusSelf,
                               pTree *treeNode, const ZZ &begRange,
                               const ZZ &endRange) {

    // Establish initial Handshakes - Check If I have nothing or If Client has
    // nothing
    int response;
    ...
    if (treeNode == nullptr || treeNode->getDatum()->getNumElem() == 0) {
        ...
        if (response == SYNC_NO_INFO) {
            ...
            if (!node->SyncServer(
                    commSync, selfMinusOther,
                    otherMinusSelf)) { // sync failure - create Children and go
                                       // try to sync

                // Accumulate stats from each CPISync in InterCPISyncs
                // mySyncStats object
                ...
                auto *tempTree = new pTree(
                    new CPISync_ExistingConnection(maxDiff, bitNum, probEps,
                                                   redundant_k, hashes),
                    pFactor);
                createChildren(treeNode, tempTree, begRange,
                               endRange); // Create child Nodes;
                treeNode =
                    tempTree; // Update the current parent node(parent node only
                              // used for referencing the child nodes)
                ZZ step = (endRange - begRange) / pFactor;
                mySyncStats.timerEnd(SyncStats::COMP_TIME);
                // if(step ==0) step = 1;
                for (int ii = 0; ii < pFactor - 1; ii++) {
                    _SyncServer(commSync, selfMinusOther, otherMinusSelf,
                                treeNode->child[ii], begRange + (ii * step),
                                begRange + (ii + 1) * step);
                } // Last child needs to handle odd pFactors
                _SyncServer(commSync, selfMinusOther, otherMinusSelf,
                            treeNode->child[pFactor - 1],
                            begRange + ((pFactor - 1) * step), endRange);
            } else {
                mySyncStats.timerStart(SyncStats::COMM_TIME);
                commSync->commSend(SYNC_OK_FLAG);
...
void InterCPISync::createChildren(pTree *parentNode, pTree *tempTree,
                                  const ZZ &begRange, const ZZ &endRange) {

    ZZ step = (endRange - begRange) /
              pFactor; // Get the step size of the node to establish bin sizes
    if (step == 0)
        step = 1; // Set minimum step size to 1 to avoid divide errors
    long pos;
    CPISync *nodes[pFactor];
    if (endRange != begRange) {
        for (int ii = 0; ii < pFactor; ii++) {
            tempTree->child[ii] =
                new pTree(new CPISync_ExistingConnection(
                              maxDiff, bitNum, probEps, redundant_k, hashes),
                          pFactor); // Create child nodes for parent
            nodes[ii] = tempTree->child[ii]
                            ->getDatum(); // Create references for the child
                                          // nodes(used for insertion)
        }
        CPISync *parent = parentNode->getDatum(); // Get the parent node

        for (auto elem = parent->beginElements(); elem != parent->endElements();
             elem++) { // Iterate through all parent information
            ZZ elemZZ = rep(_hash(*elem));
            pos = pFactor - 1;
            for (int jj = 0; jj < pFactor - 1; jj++) {
                if ((elemZZ >= (begRange + (jj * step))) &&
                    (elemZZ < (begRange + (jj + 1) * step))) {
                    pos = jj;
                    break;
                }
            }
...
bool InterCPISync::_SyncClient(const shared_ptr<Communicant> &commSync,
    ...
    try {
        ...
        if (response ==
            ...
            // Accumulate stats from each CPISync in InterCPISyncs mySyncStats
            // object
            mySyncStats.increment(SyncStats::XMIT,
                                  node->mySyncStats.getStat(SyncStats::XMIT));
            mySyncStats.increment(SyncStats::RECV,
                                  node->mySyncStats.getStat(SyncStats::RECV));
            mySyncStats.increment(
                SyncStats::COMM_TIME,
                node->mySyncStats.getStat(SyncStats::COMM_TIME));
            ...
            if (commSync->commRecv_byte() ==
                ...
                createChildren(treeNode, tempTree, begRange,
                               endRange); // Create child Nodes;
                treeNode = tempTree;      // Update the current parent node(temp
                                     // parent only children are used)
                ZZ step = (endRange - begRange) / pFactor;
                mySyncStats.timerEnd(SyncStats::COMP_TIME);
